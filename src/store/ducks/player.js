...
export default function player(state = INITIAL_STATE, action) {
  switch (action.type) {
    ...
    case Types.PLAYING:
      // could be passed as { ...state, ...action.paylod } too
      return { ...state, position: action.payload.position, duration: action.payload.duration };
    case Types.HANDLE_POSITION:
      // set the positionShown based in the percent multiplied by the duration
      return { ...state, positionShown: state.duration * action.payload.percent };
    case Types.SET_POSITION:
      // set the position based in the percent multiplied by the duration
